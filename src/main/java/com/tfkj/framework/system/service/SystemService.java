...
import com.tfkj.framework.core.security.Digests;
 ...
 * 
 * <Br>
 * TODO：系统用户 操作方法需调用permissionClient 接口自行实现
 * 
 * @author ThinkGem
 * @version 2013-12-05
 */
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public List<User> findUser(User user) {
		...
		/*
		 * user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(),
		 * "o", "a")); List<User> list = userDao.findList(user);
		 */
	...
	public List<User> findUserByOfficeId(String officeId) {
		...
		/*
		 * if (list == null) { User user = new User(); user.setOffice(new
		 * Office(officeId)); list = userDao.findUserByOfficeId(user);
	...
	public void saveUser(User user) {
		...
		/*
		 * user.setType("01"); if (StringUtils.isBlank(user.getId()) ||
		 * user.getIsNewRecord()) { if (StringUtils.isBlank(user.getId())) {
		 * user.preInsert(); } userDao.insert(user); } else { // 清除原用户单位用户缓存
		 * User oldUser = userDao.get(user.getId()); if (oldUser != null &&
		 * oldUser.getOffice() != null && oldUser.getOffice().getId() != null) {
		 * CacheUtils.remove(UserUtils.USER_CACHE,
		 * UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ +
		 * oldUser.getOffice().getId()); } // 更新用户数据 user.preUpdate();
		 * userDao.update(user); } if (StringUtils.isNotBlank(user.getId())) {
		 * // 更新用户与角色关联 userDao.deleteRoleUser(user); if (user.getRoleList() !=
		 * null && user.getRoleList().size() > 0) {
		 * userDao.insertRoleUser(user); } else { throw new
		 * ServiceRuntimeException(user.getLoginName() + "没有设置角色！"); }
		 */
		// 清除用户缓存
	...
	public void updateUserInfo(User user) {
		...
		/*
		 * user.preUpdate(); userDao.updateUserInfo(user);
		 */
		// 清除用户缓存
		UserUtils.clearCache(user);
		// // 清除权限缓存
	...
	public void updatePasswordById(String id, String loginName,
			String newPassword) {
		/*
		 * User user = new User(id); user.setPassword(newPassword);
		 * user.preUpdate(); userDao.updatePasswordById(user);
		 */
		// 清除用户缓存
		/*
		 * user.setLoginName(loginName); UserUtils.clearCache(user);
		 */
		// // 清除权限缓存
		// systemRealm.clearAllCachedAuthorizationInfo();
	}
	...
	public void updateUserLoginInfo(User user) {
		...
		/*
		 * // 保存上次登录信息 user.setOldLoginIp(user.getLoginIp());
		 * user.setOldLoginDate(user.getLoginDate()); // 更新本次登录信息
		 * user.setLoginIp(StringUtils.getRemoteAddr(Servlets.getRequest()));
		 * user.setLoginDate(new Date()); user.preUpdate();
	...
	public static boolean validatePassword(String plainPassword, String password) {
		 ...
		 * Digests.sha1(plain.getBytes(), salt, HASH_INTERATIONS); return
		 * password.equals(Encodes.encodeHex(salt) +
		...
		if (password.trim().equals(plainPassword.trim())) {
			return true;
		}
		return false;
	}
	...
	public Boolean outUserInRole(Role role, User user) {
		...
		/*
		 * List<Role> roles = user.getRoleList(); for (Role e : roles) { if
		 * (e.getId().equals(role.getId())) { roles.remove(e); saveUser(user);
		 * return true; } }
		 */
		return false;
	}
	...
	public User assignUserToRole(Role role, User user) {
		...
		/*
		 * if (user == null) { return null; } List<String> roleIds =
		 * user.getRoleIdList(); if (roleIds.contains(role.getId())) { return
		 * null; } user.getRoleList().add(role); saveUser(user);
		 */
		return user;
	}
