        ...
        function save() {
            ...
            if (inspectionTask){
                ...
                $.ajax({
                    type: "post",
                    data: obj,
                    url: "${ctx}/inspection/save",
                    dataType: "json",
                    success: function (res) {
                        if(res.success=='1'){
                            $("#id").val(res.id);
                            alertx("保存成功！");
                            window.location.href = "${ctx}/inspection/index";
                        }else{
                            alertx("操作失败！");
                        }
                    },
...
//            }
        ...
        function findDictSelect() {
            $.ajax({
                type: "post",
                data: "",
                url: "${ctx}/sys/dict/dictTree?type=RWLX",
                dataType: "json",
                success: function (res) {
                    if(res){
                        $("<option value=''></option>").appendTo($("#taskType"));
                        $.each(res, function (i, v) {
                            var value ='${table.taskType}';
                            if (value!=null&&value!=''&&value==v.id){
                                $("<option selected='selected' value='" + v.id + "'>" + v.name  + "</option>").appendTo($("#taskType"));
                            }else{
                                $("<option value='" + v.id + "'>" + v.name  + "</option>").appendTo($("#taskType"));
                            }
...
<div >
    <form id="inspectionForm" class="form-horizontal">
        ...
        <div class="control-group">
            <label class="control-label" >带队领导</label>
            <div class="controls" >
                <sys:treeselect cssStyle="width:630px" id="leadLeader" name="leadLeader" value="${table.leadLeader}" labelName="leadLeaderText" hideBtn="true"
                                labelValue="${table.leadLeaderText}" title="领导" url="/sys/leader/leaderTree?type=01" cssClass="required" checked="true" allowClear="true" />
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" >参加单位</label>
            <div class="controls" >
                <sys:treeselect cssStyle="width:630px" id="joinCompany" name="joinCompany" value="${table.joinCompany}" labelName="joinCompanyText" labelValue="${table.joinCompanyText}" notAllowSelectParent = "true"
        ...
        <div class="control-group">
            ...
            <div class="controls" >
                <sys:officeTreeFileTime id="inspectionRange" leftText2="上传督查专函" leftText3="上传回复报告" isShowEndTime="true" name="inspectionRangeList" value="${table.inspectionRangeList}" title="单位" isShowRemark="true"
