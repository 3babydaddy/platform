...
//                    },
                    {
                        ...
                        formatter: function (value, row) {
                            ...
                            if (value.length>5){
                                ...
                                return "<nobr>" +
                                    "<a title = '"+value+"' href='${ctx}/inspection/form?id=" + row.id + "'>" + manager + "</a>"
                                    + "</nobr>"
                            }else{
                                return "<nobr>" +
                                    "<a href='${ctx}/inspection/form?id=" + row.id + "'>" + value + "</a>"
                                    + "</nobr>"
                            }
                        ...
                        formatter: function (value, row) {
                            ...
                            if (value){
                                var array = value.split(/[,，、]/);
                                if(array.length>1){
                                    manager = array[0] + "等"+array.length+"家单位";
                                }else{
                                    manager = value;
                                }
                            }
        ...
        // 新添
        function add() {
            window.location.href = "${ctx}/inspection/form";
        }
        function delect() {
            ...
            if(rows.length>0){
                confirmx("确认要删除该记录吗?",function () {
                    $.ajax({
                        type: "post",
                        data: '',
                        url: "${ctx}/inspection/delete?ids="+ids,
                        cache: false,
                        dataType: "json",
        ...
        function update(id) {
                window.location.href = "${ctx}/inspection/form?id="+id;
        }
        function saveSort() {
      ...
      //导出excel
        function exportExcel() {
            confirmx('确认要导出数据吗？',function(){
             ...
             window.location.href="${ctx}/inspection/exportExcel?ids="+ids+"&"+decodeParams;
            },'')
        }
    </script>
    <style type="text/css">
...
<body>
<ul class="breadcrumb">
    <li class="active">专项督查</li>

</ul>
<div>
    <form:form id="searchForm" modelAttribute="specialSupervision" action="${ctx}/inspection/list" method="post">
        <div class="controls controls-row">
            <div class="search-div input-prepend input-append">
                <span class="add-on">任务类型</span>
                <form:select path="taskType" class="search-select-medium">
                    <option value="">全部</option>
                    <%-- <form:options items="${fns:getDictList('RWLX')}" itemLabel="chname" itemValue="enname" htmlEscape="false" /> --%>
                </form:select>
            </div>
            <div class="search-div input-prepend input-append margin-left-1">
                <span class="add-on">带队领导</span>
            ...
            <div class="search-div input-prepend input-append">
                ...
                <input class="search-input-mini" type="text" name="inspectionDate" value="<fmt:formatDate value="${inspectionDate}" pattern="yyyy-MM-dd" />" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
                <span class="add-on">至</span>
                <input class="search-input-mini" type="text" name="inspectionEndDate" value="<fmt:formatDate value="${inspectionEndDate}" pattern="yyyy-MM-dd" />" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
            </div>
            <div class="search-div input-prepend input-append">
        ...
        <div class="controls controls-row">
            ...
            <div class="search-div input-prepend input-append margin-left-1">
                <span class="add-on">参加单位</span>
                <sys:treeselect id="joinCompany" name="joinCompany" value="${table.joinCompany}" labelName="joinCompanyText" labelValue="${table.joinCompanyText}" title="参加单位" url="/sys/office/treeData?isAll=true" cssClass="required" allowClear="true" notAllowSelectParent="true" hideBtn="true" />
            </div>
            <div class="search-div input-prepend input-append">
