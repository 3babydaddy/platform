...
//                    },{
                    {
                        ...
                        formatter: function (value, row) {
                            ...
                            if (value.length>5){
                                ...
                                return "<nobr>" +
                                    "<a title = '"+value+"' href='${ctx}/instructions/form?id=" + row.id + "'>" + manager + "</a>"
                                    + "</nobr>"
                            }else{
                                return "<nobr>" +
                                    "<a href='${ctx}/instructions/form?id=" + row.id + "'>" + value + "</a>"
                                    + "</nobr>"
                            }
        ...
        // 新添
        function add() {
            window.location.href = "${ctx}/instructions/form";
        }
        function delect() {
            ...
            if(rows.length>0){
                confirmx("确认要删除该记录吗?",function () {
                    $.ajax({
                        type: "post",
                        data: '',
                        url: "${ctx}/instructions/delete?ids="+ids,
                        cache: false,
                        dataType: "json",
        ...
        function update(id) {
                window.location.href = "${ctx}/instructions/form?id="+id;
        }
        function saveSort() {
        ...
        function exportExcel() {
            confirmx('确认要导出数据吗？',function(){
                ...
                window.location.href="${ctx}/instructions/exportExcel?ids="+ids+"&"+decodeParams;
            },'')
        }
    </script>
    <style type="text/css">
...
<body>
<ul class="breadcrumb">
    <li class="active">专项核查</li>

</ul>
<div>
    <form:form id="searchForm" modelAttribute="specialSupervision" action="${ctx}/instructions/list" method="post">
        <div class="controls controls-row">

            <div class="search-div input-prepend input-append">
                <span class="add-on">核查事项</span>
                <form:input cssClass="search-input-medium" path="inspectionTask" htmlEscape="false" maxlength="50" />
            </div>
            <div class="search-div input-prepend input-append ">
                <span class="add-on">核查时间</span>
