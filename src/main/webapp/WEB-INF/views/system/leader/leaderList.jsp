...
<script type="text/javascript">
    $(function() {
		...
		$("#batchUpdateBtn").click(function() {
			loading('请稍等...');
			$("#listForm").attr("action", "${ctx}/sys/leader/batchUpdate");
			$("#listForm").submit();
		});
        var tpl = $("#treeTableTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
        var data = ${fns:toJson(list)}, rootId = "${not empty leader.id ? leader.id : '0'}";
...
<body>
	<ul class="breadcrumb">
		<li class="active">
			<a href="${ctx}/sys/leader/">领导信息列表</a>
		</li>
	</ul>
	<div id="toolbar">
		<ul class="nav nav-pills">
			...
			<li><a id="add" href="${ctx}/sys/leader/form"><i class="icon-plus"></i>&nbsp;新增</a></li>

		</ul>
	</div>
	<div id="searchbar" class="form-search breadcrumb">
		<form:form id="searchForm" modelAttribute="leader" action="${ctx}/sys/leader/cxlist" method="post" style="margin:0px;">
		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed" style="width: 60%">
			<thead>
				<tr>
					...
					<shiro:hasPermission name="sys:leader:edit">
	...
	<script type="text/template" id="treeTableTpl">
        <tr id="{{row.id}}" pId="{{pid}}">
            <td><a href="${ctx}/sys/leader/form?id={{row.id}}">{{row.l001}}</a></td>
			...
			<td>
                <a href="${ctx}/sys/leader/form?id={{row.id}}">修改</a>
